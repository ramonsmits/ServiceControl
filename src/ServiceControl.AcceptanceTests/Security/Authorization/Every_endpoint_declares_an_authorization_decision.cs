namespace ServiceControl.AcceptanceTests.Security.Authorization
{
    ...
    class Every_endpoint_declares_an_authorization_decision : AcceptanceTest
    {
        ...
        public async Task All_endpoints_have_a_reviewed_authorization_decision()
        {
            ...
            // Enumerate endpoints inside Done() while the host (and its DI container) is still running.
            // Reading dataSource.Endpoints after Run() returns causes ObjectDisposedException because
            // RouteEndpointDataSource.get_Endpoints() re-enters the DI container to resolve metadata.
