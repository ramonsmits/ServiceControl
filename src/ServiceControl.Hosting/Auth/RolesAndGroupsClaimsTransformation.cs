...
/// are not added again. In XACML terms this is a <strong>PIP (Policy Information Point)</strong>
/// for subject attributes: it converts opaque IdP-supplied claim shapes into the canonical
/// role/group claims the PDP (<see cref="ServiceControl.Infrastructure.Auth.Rbac.IPermissionEvaluator"/>)
/// reads directly. See <c>research/platform-authorization/xacml-vocabulary.md</c> for the full vocabulary.
/// </para>
/// </remarks>
public class RolesAndGroupsClaimsTransformation(string rolesClaimPath, string groupsClaimPath) : IClaimsTransformation
{
    ...
    static List<string> ExtractFromJsonBlob(string json, string jsonPath)
    {
        ...
        {
            // Malformed claim — skip transformation rather than throw at request time
            return [];
        }
    }
