...
public class RolesAndGroupsClaimsTransformationTests
{
    ...
    public async Task Flat_claim_with_json_array_value_is_unpacked()
    {
        ...
        var identity = new ClaimsIdentity("Bearer");
        // Some Entra deployments deliver the multi-value claim as a single JSON-array string
    ...
    public async Task Cognito_groups_flat_claim_with_colon_in_name()
    {
        ...
        // cognito:groups is configured for both roles and groups in this test, so both
        // canonical claim types should be populated independently and idempotently.
