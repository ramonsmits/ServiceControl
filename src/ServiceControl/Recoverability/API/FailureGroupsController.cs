namespace ServiceControl.Recoverability.API
{
        ...
        : ControllerBase
    {
        ...
        /// grants for <c>recoverabilitygroups:view</c>, access is denied with 403 — the comment
        /// could expose group metadata from queues outside the user's scope.
        /// </para>
        /// </summary>
        [Authorize(Policy = Permissions.RecoverabilityGroupsView)]
        ...
        /// <summary>
        /// Fail-closed group scope check. Returns a 403 <see cref="IActionResult"/> if the user
        /// has only scope-restricted grants for <paramref name="permission"/>; returns
        /// <see langword="null"/> if the user has an unrestricted grant (access allowed).
        /// <para>
        /// Groups span multiple queues and cannot be scope-checked against a single queue address.
        /// A user with only scoped grants is denied fail-closed to prevent exposure of group
        /// data from queues outside their scope.
        /// </para>
        /// </summary>
